# Lab Activity: Module 02 - Network Sec
...
3. On Kali Linux, open a terminal and run a basic Nmap scan against the Windows IP:
   `nmap -sV -O [Windows_IP]`
4. This command runs a version scan (`-sV`) to find out what services are running on open ports, and an OS detection scan (`-O`).
5. Review the output to see what ports are open (e.g., Port 3389 for RDP, Port 445 for SMB).
**Deliverable:** Take a screenshot of the Kali terminal showing the completed Nmap scan and the list of open ports on the target Windows server. Submit to Blackboard.
