...
## Objective
Configure and verify systems matching the operational parameters of **Advanced OOP: Inheritance and Polymorphism**.
...
1. **Create a base class `Vehicle` and subclass `Car`**
   * *Instruction:* Execute this step inside your terminal environment. Verify the command completes without errors.
2. **Override a method `start_engine` in `Car`**
   * *Instruction:* Execute this step inside your terminal environment. Verify the command completes without errors.
3. **Use `super().__init__()` to inherit initialization attributes**
   * *Instruction:* Execute this step inside your terminal environment. Verify the command completes without errors.
